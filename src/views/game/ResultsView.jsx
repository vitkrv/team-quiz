...
export default function ResultsView({ room, gameId, leaveRoom }) {
    const { t } = useLanguage();
    ...
    return (
        <div className="min-h-screen bg-slate-900 flex flex-col items-center p-6 pt-8 md:pt-10">
            <h1 className="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-600 mb-6 drop-shadow-lg text-center">
                {t('gameOver')}
            </h1>

            {winner ? (
                <div className="flex w-full max-w-2xl min-w-0 flex-col items-center mb-8 animate-in slide-in-from-bottom-10 duration-700">
                    <div className="flex items-center gap-4 mb-3">
                        <Trophy size={64} className="text-yellow-400 drop-shadow-[0_0_30px_rgba(250,204,21,0.5)]" />
                        <div className="h-14 w-14 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-4xl shadow-lg shadow-black/30">
                            {winner.avatar}
                        </div>
                    </div>
                    <h2 className="max-w-full break-words text-3xl md:text-4xl font-bold text-white mb-2 text-center">{winner.name}</h2>
                    <p className="text-xl font-mono text-yellow-400">{t('scorePts', { score: winner.score })}</p>
                </div>
            ) : (
                <div className="text-2xl text-slate-400 mb-16">{t('noPlayersParticipated')}</div>
            )}

            <div className="w-full max-w-2xl bg-slate-800 rounded-2xl border border-slate-700 p-4 md:p-6 mb-6">
                <h3 className="text-xl font-bold text-slate-400 mb-4 uppercase tracking-widest text-center">{t('finalStandings')}</h3>
                <div className="space-y-2">
                    {playerEntries.map(([playerId, p], i) => (
                        <div key={playerId} className="flex justify-between items-center gap-4 p-3 bg-slate-900 rounded-xl">
            ...
            {room.recapVersion === 1 && <ResultsRecap key={gameId} room={room} gameId={gameId} playerEntries={playerEntries} />}
